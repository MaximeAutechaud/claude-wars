...
static func find_nearest_enemy(unit: Unit, units_layer: UnitsLayer) -> Unit:
	...
	for child in units_layer.get_children():
		...
		if u.team == unit.team:
			continue
		var d := Pathfinder.manhattan(u.cell, unit.cell)
		if d < best_dist:
			best_dist = d
			nearest = u
	return nearest
...
static func best_move_towards(unit: Unit, target_cell: Vector2i,
	...
	var best_gap := absi(Pathfinder.manhattan(unit.cell, target_cell) - r)
	var best_d := Pathfinder.manhattan(unit.cell, target_cell)
	for cell: Vector2i in reachable:
		if cell == unit.cell:
			continue
		if units_layer.get_unit_at(cell) != null:
			continue  # case occupée (allié ou ennemi)
		var d := Pathfinder.manhattan(cell, target_cell)
		var gap := absi(d - r)
